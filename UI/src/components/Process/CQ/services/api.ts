export async function parseCsvFile(csvFile: File) {
  ...
  const response = await fetch("http://127.0.0.1:5000/parse_csv", {
    method: "POST",
    body: formData,
  });
...
export async function savePoints(points: any[], dates: string[], metadata: any, duration: number) {
  const response = await fetch("http://127.0.0.1:5000/save_points", {
    method: "POST",
...
export async function updateMetadata(metadata: any) {
  const response = await fetch("http://127.0.0.1:5000/update_metadata", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
