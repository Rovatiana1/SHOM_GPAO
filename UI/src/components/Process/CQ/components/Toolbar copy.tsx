...
const Toolbar: React.FC<ToolbarProps> = ({
    ...
    const baseIsDisabled = isProcessing || processLoading;

    const csvInputRef = useRef<HTMLInputElement>(null);

    const handleImportOrSelect = () => {
        if (csvFile) {
    ...
    const isAutoLoaded = autoLoadedFilename && csvFile?.name === autoLoadedFilename;

    // The button is disabled if a process is running.
    // For non-admins, it's also disabled if a file is auto-loaded (they can't change it).
    // An admin can always select a new file, even if one is auto-loaded.
    const importDisabled = baseIsDisabled || (!isAdmin && isAutoLoaded);
    const buttonText = csvFile ? "Import" : "Sélectionner un fichier";
            ...
            <div className="border-t border-b border-gray-200 py-4">
                <div className="space-y-3">
                    ...
                    <div
                        ...
                        onClick={() => !importDisabled && csvInputRef.current?.click()}
                    >
                        {csvFile ? (
                            <div className="text-center">
                                <p className="text-sm text-gray-600 mx-auto w-[75%] truncate">{csvFile.name}</p>
                                {csvFile.size ? <p className="text-xs text-gray-500 mt-1">{(csvFile.size / 1024).toFixed(2)} KB</p> : null}
