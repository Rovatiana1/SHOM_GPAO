...
const CanvasComponent: React.FC<CanvasProps> = ({
    ...
    // Effect for escape key to cancel capture
    useEffect(() => {
    ...
    useEffect(() => {
        ...
        ctx.drawImage(image, 0, 0);
        // In capture mode, we only want to show the image, not the points or guides.
        if (isCaptureMode) {
            ctx.restore();
            // We don't draw the selection rectangle here on the canvas because
            // it's handled by a separate DOM element overlay.
            return;
        }
    ...
    }, [image, points, dates, viewTransform, displayMode, selectedDate, dateColorMap, uniqueDates, guides, metadata, repereStep, tempRepere, addingPoint, repereMousePos, isCaptureMode]);
    ...
    const handleCaptureMouseDown = (e: React.MouseEvent<HTMLDivElement>) => {
        ...
        // Prevent pan/zoom while capturing
        e.stopPropagation();
        ...
        // Prevent starting selection outside of the actual canvas area
    ...
    const handleCaptureMouseUp = () => {
        ...
        // Ignore tiny selections that might be accidental clicks
        ...
        // Transform viewport coordinates of the selection to image coordinates
        ...
        // Draw the selected part of the source image, not the canvas with points/lines
        ...
        <div className="w-full h-full flex items-center justify-center bg-gray-800 relative">
            ...
            {isCaptureMode && (
                <div
                    ...
                    onMouseLeave={() => setIsSelecting(false)} // Stop selection if mouse leaves area
                >
                    ...
                    {selectionRect && (
                        <div
                            ...
                            style={{
                                ...getSelectionStyle(),
                                // This div is relative to the overlay, which is larger than the canvas.
                                // We need to offset it to align with the canvas.
                                // FIX: Property 'left' does not exist on type '{ startX: number; startY: number; endX: number; endY: number; }'. Used Math.min(selectionRect.startX, selectionRect.endX) instead.
                                left: `${(canvasRef.current?.getBoundingClientRect().left ?? 0) - (canvasRef.current?.parentElement?.getBoundingClientRect().left ?? 0) + (Math.min(selectionRect.startX, selectionRect.endX)) }px`,
                                // FIX: Property 'top' does not exist on type '{ startX: number; startY: number; endX: number; endY: number; }'. Used Math.min(selectionRect.startY, selectionRect.endY) instead.
                                top: `${(canvasRef.current?.getBoundingClientRect().top ?? 0) - (canvasRef.current?.parentElement?.getBoundingClientRect().top ?? 0) + (Math.min(selectionRect.startY, selectionRect.endY)) }px`,
                            }}
                         />
                    )}
                </div>
