...
// FIX: Implement preview mode functionality. Added imports for PreviewCanvasComponent and InterpolationService.
...
const RepriseCQ: React.FC = () => {
    ...
    // FIX: Add isExporting state for the main export process
    const [isExporting, setIsExporting] = useState(false);
    // FIX: Implement preview mode functionality. Added state variables for preview mode.
    ...
    const resetState = useCallback(() => {
        ...
        // FIX: Implement preview mode functionality. Reset preview state.
        setPreviewData(null);
    ...
    // FIX: Implement preview mode functionality. Added handlers for preview mode.
    ...
    const handleSaveCapture = (type: string, nature: string) => {
        ...
        const newCaptures = captures.filter(c => c.isNew && c.type === type);
        const newIndexStr = (newCaptures.length + 1).toString().padStart(3, '0');

        const suffix = type === "Anomalie" ? "AN" : "MC";
        const filename = `${nextCaptureInfo.baseName}_${suffix}_${newIndexStr}.jpg`;
    ...
    const handleDeleteCapture = (indexToDelete: number) => {
        const captureToDelete = captures[indexToDelete];
        if (!captureToDelete?.isNew) {
            setToast({ message: "Les captures existantes ne peuvent pas être supprimées.", type: 'error' });
            return;
        }
        setCaptures(prevCaptures => prevCaptures.filter((_, i) => i !== indexToDelete));
    };

    const handleArchiveCapture = async (captureId: number) => {
        if (!currentLot || !currentLot?.lotCQ) return;
        try {
            const res = await CQService.archiveCapture(currentLot?.lotCQ.idLot, captureId);
            console.log(res);
            setCaptures(prev => prev.map(c => c.id === captureId ? { ...c, isArchived: true } : c));
    ...
    const handleExportCaptures = async () => {
        const newCapturesToExport = captures.filter(c => c.isNew);
        if (newCapturesToExport.length === 0) {
            setToast({ message: "Aucune nouvelle capture à exporter.", type: 'info' });
            return;
        }
        if (!currentLot || !paths?.out || !currentLot?.lotCQ) {
        ...
        const allCapturesForCSV = captures.filter(c => !c.isArchived);

        let csvContent = "image correspondante;nom de l'image;type;nature\n";
        allCapturesForCSV.forEach(c => {
            csvContent += `${mainImageName};${c.filename};"${c.type}";"${c.nature}"\n`;
        });

        try {
            const response = await CQService.saveCaptures(csvContent, allCapturesForCSV, paths.out, currentLot?.lotCQ.idLot, mainImageName);
            setToast({ message: `Nouvelles captures exportées: ${response.message}`, type: 'success' });
            setCaptures(prev => prev.filter(c => !c.isNew)); // Clear new captures after export
        } catch (e) {
    ...
    return (
        <div className="flex h-full w-full font-sans">
            ...
            {/* FIX: Removed the `newCaptureCount` prop as it does not exist on `ToolbarProps`. The `Toolbar` component does not use this prop, so its removal does not affect functionality. */}
            <Toolbar
                ...
                captureCount={captures.filter(c => !c.isArchived).length}
            ...
            <main className="relative flex-1 flex items-center justify-center bg-gray-200 p-4 overflow-hidden h-[90vh]">
                {/* FIX: Implement preview mode functionality. Added isPreviewLoading to loading overlay condition. */}
                ...
                {/* FIX: Implement preview mode functionality. Conditionally render PreviewCanvasComponent or CanvasComponent. */}
            ...
            {isCaptureModalOpen && capturedImage && nextCaptureInfo && (
                <CaptureModal
                    ...
                    captures={captures.filter(c => c.isNew === true)}
                />
            )}
            ...
            />
            {/* FIX: Implement preview mode functionality. Added DurationModal for preview. */}
            <DurationModal
