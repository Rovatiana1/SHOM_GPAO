...
import { X, Save, Loader, Plus, User, Edit, Trash2, AlertTriangle, ArrowLeft } from 'lucide-react';
...
interface CrudPanelProps {
    ...
    onAddNew: () => void;
    onCancel: () => void;
    onSave: (userData: Partial<ManagedUser>) => Promise<boolean>;
    onDelete: () => void;
    onEdit: (user: ManagedUser) => void;
    onConfirmDelete: (user: ManagedUser) => void;
}
...
const CrudPanel: React.FC<CrudPanelProps> = ({ panelState, user, onAddNew, onCancel, onSave, onDelete, onEdit, onConfirmDelete }) => {
    
    const renderContent = () => {
        switch(panelState) {
            case 'viewUser':
                return user && <ViewPanel user={user} onEdit={onEdit} onConfirmDelete={onConfirmDelete} onCancel={onCancel} />;
            case 'newUser':
            case 'editUser':
                return <FormPanel userToEdit={user} onSave={onSave} onCancel={onCancel} />;
            case 'deleteUser':
                return user && <DeletePanel user={user} onDelete={onDelete} onCancel={onCancel} />;
            case 'idle':
            default:
...
const IdlePanel: React.FC<{ onAddNew: () => void }> = ({ onAddNew }) => (
    <div className="flex flex-col items-center justify-center h-full p-8 text-center bg-gray-50">
        ...
        <h3 className="text-lg font-medium text-gray-700">Gestion des utilisateurs</h3>
        <p className="text-sm text-gray-500 mt-1">Sélectionnez un utilisateur pour voir ses détails ou en ajouter un nouveau.</p>
        <button
            onClick={onAddNew}
...
const ViewPanel: React.FC<{ user: ManagedUser, onEdit: (user: ManagedUser) => void, onConfirmDelete: (user: ManagedUser) => void, onCancel: () => void }> = ({ user, onEdit, onConfirmDelete, onCancel }) => {
        ...
        <div className="flex flex-col h-full">
            <div className="p-6 border-b border-gray-200 flex items-center gap-4">
                ...
                <div className="flex-shrink-0 h-12 w-12 rounded-full bg-green-100 flex items-center justify-center">
                    <span className="font-bold text-green-700 text-xl">{getInitials(user.prenom, user.nom)}</span>
                </div>
                <div>
            ...
            <div className="flex-1 p-6 space-y-4 overflow-y-auto">
                <InfoItem label="Login LDAP" value={user.ldap_name} />
                <InfoItem label="ID Interne" value={user.id_pers.toString()} />
                <div>
                    <p className="text-sm font-medium text-gray-500">Rôles</p>
                    <div className="flex flex-wrap gap-2 mt-2">
                        {user.roles?.length > 0 ? user.roles.map(role => (
                            <span key={role} className={`px-2.5 py-0.5 inline-flex text-xs leading-5 font-semibold rounded-full ${roleColors[role] || 'bg-gray-100 text-gray-800'}`}>
                                {role}
                            </span>
                        )) : <p className="text-sm text-gray-600">Aucun rôle assigné</p>}
                    </div>
                </div>
            </div>
            <div className="p-4 bg-gray-50 border-t border-gray-200 flex justify-end gap-3">
                <button onClick={() => onConfirmDelete(user)} className="flex items-center gap-2 px-4 py-2 text-sm font-medium rounded-md text-red-700 bg-red-100 hover:bg-red-200 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-500">
                    <Trash2 size={16} /> Supprimer
                </button>
                <button onClick={() => onEdit(user)} className="flex items-center gap-2 px-4 py-2 text-sm font-medium rounded-md text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">
...
const FormPanel: React.FC<{ userToEdit: ManagedUser | null; onSave: (data: Partial<ManagedUser>) => Promise<boolean>; onCancel: () => void }> = ({ userToEdit, onSave, onCancel }) => {
    const [formData, setFormData] = useState({ nom: '', prenom: '', ldap_name: '', email: '' });
    const [roles, setRoles] = useState<Set<UserRole>>(new Set());
    const [isSubmitting, setIsSubmitting] = useState(false);

    const isEditMode = userToEdit !== null;
    const ALL_ROLES: UserRole[] = ['ADMIN', 'MANAGER', 'USER'];

    useEffect(() => {
        if (userToEdit) {
            setFormData({ nom: userToEdit.nom || '', prenom: userToEdit.prenom || '', ldap_name: userToEdit.ldap_name || '', email: userToEdit.email || '' });
            setRoles(new Set(userToEdit.roles || []));
        } else {
            setFormData({ nom: '', prenom: '', ldap_name: '', email: '' });
            setRoles(new Set(['USER'])); // Default to USER role for new users
        }
    }, [userToEdit]);
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        const payload = { ...formData, roles: Array.from(roles) };
        const success = await onSave(payload);
        if (!success) {
            setIsSubmitting(false); // Only stop submitting if save failed
        }
    };
    ...
    return (
        <form onSubmit={handleSubmit} className="flex flex-col h-full">
            <div className="flex justify-between items-center p-4 border-b">
                <h2 className="text-xl font-semibold text-gray-800">{isEditMode ? 'Modifier' : 'Ajouter'} un utilisateur</h2>
            ...
            <div className="p-6 flex-1 space-y-4 overflow-y-auto">
                ...
                <InputField label="Login LDAP" name="ldap_name" value={formData.ldap_name} onChange={handleChange} required />
                <div>
                    <label className="block text-sm font-medium text-gray-700">Rôles</label>
                    <div className="mt-2 grid grid-cols-3 gap-4">
                        {ALL_ROLES.map(role => (
                            <label key={role} className="flex items-center">
                                <input type="checkbox" checked={roles.has(role)} onChange={() => handleRoleChange(role)} className="h-4 w-4 rounded border-gray-300 text-green-600 focus:ring-green-500" />
...
const DeletePanel: React.FC<{ user: ManagedUser, onDelete: () => void, onCancel: () => void }> = ({ user, onDelete, onCancel }) => {
    ...
    return (
        <div className="flex flex-col h-full">
             <div className="flex items-center p-4 border-b">
                 <h2 className="text-xl font-semibold text-gray-800">Supprimer l'utilisateur</h2>
            </div>
