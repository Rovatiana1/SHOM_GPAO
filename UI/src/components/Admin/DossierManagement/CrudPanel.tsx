...
import { X, Save, Loader, Plus, Folder, Edit, Trash2, AlertTriangle, ArrowLeft } from 'lucide-react';
...
interface CrudPanelProps {
    ...
    onAddNew: () => void;
    onCancel: () => void;
    onSave: (dossierData: Partial<Dossier>) => Promise<boolean>;
    onDelete: () => void;
    onEdit: (dossier: Dossier) => void;
    onConfirmDelete: (dossier: Dossier) => void;
}
...
const CrudPanel: React.FC<CrudPanelProps> = ({ panelState, dossier, onAddNew, onCancel, onSave, onDelete, onEdit, onConfirmDelete }) => {
    
    const renderContent = () => {
        switch(panelState) {
            case 'viewDossier':
                return dossier && <ViewPanel dossier={dossier} onEdit={onEdit} onConfirmDelete={onConfirmDelete} onCancel={onCancel} />;
            case 'newDossier':
            case 'editDossier':
                return <FormPanel dossierToEdit={dossier} onSave={onSave} onCancel={onCancel} />;
            case 'deleteDossier':
                return dossier && <DeletePanel dossier={dossier} onDelete={onDelete} onCancel={onCancel} />;
            case 'idle':
            default:
...
// --- Sub-components for different panel states ---

const IdlePanel: React.FC<{ onAddNew: () => void }> = ({ onAddNew }) => (
    <div className="flex flex-col items-center justify-center h-full p-8 text-center bg-gray-50">
        <Folder className="w-16 h-16 text-gray-300 mb-4" />
        <h3 className="text-lg font-medium text-gray-700">Gestion des dossiers</h3>
        <p className="text-sm text-gray-500 mt-1">Sélectionnez un dossier pour voir ses détails ou en ajouter un nouveau.</p>
        <button
            onClick={onAddNew}
        ...
        >
            <Plus size={18} /> Ajouter un dossier
        </button>
    </div>
);

const ViewPanel: React.FC<{ dossier: Dossier, onEdit: (dossier: Dossier) => void, onConfirmDelete: (dossier: Dossier) => void, onCancel: () => void }> = ({ dossier, onEdit, onConfirmDelete, onCancel }) => {
        ...
        <div className="flex flex-col h-full">
            <div className="p-6 border-b border-gray-200 flex items-center gap-4">
                ...
                <div className="flex-shrink-0 h-12 w-12 rounded-lg bg-green-100 flex items-center justify-center">
                    <span className="font-bold text-green-700 text-sm">D{dossier.idDossier}</span>
                </div>
                <div>
                    <h2 className="text-xl font-bold text-gray-800">{dossier.numDossier}</h2>
                    <p className="text-sm text-gray-500">{dossier.alias || 'Aucun alias'}</p>
                </div>
            </div>
            <div className="flex-1 p-6 space-y-4 overflow-y-auto">
                <InfoItem label="Atelier" value={dossier.atelier || 'N/A'} />
                <InfoItem label="Correspondant" value={dossier.corresp || 'N/A'} />
                <InfoItem label="Volume Prévu" value={dossier.volumePrevue || 'N/A'} />
                <div>
                    <p className="text-sm font-medium text-gray-500">État</p>
                    <div className="mt-2">{getStatusBadge(dossier.idEtat)}</div>
                </div>
            </div>
            <div className="p-4 bg-gray-50 border-t border-gray-200 flex justify-end gap-3">
                <button onClick={() => onConfirmDelete(dossier)} className="flex items-center gap-2 px-4 py-2 text-sm font-medium rounded-md text-red-700 bg-red-100 hover:bg-red-200 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-500">
                    <Trash2 size={16} /> Supprimer
                </button>
                <button onClick={() => onEdit(dossier)} className="flex items-center gap-2 px-4 py-2 text-sm font-medium rounded-md text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">
...
const FormPanel: React.FC<{ dossierToEdit: Dossier | null; onSave: (data: Partial<Dossier>) => Promise<boolean>; onCancel: () => void }> = ({ dossierToEdit, onSave, onCancel }) => {
    const [formData, setFormData] = useState<Partial<Dossier>>({});
    const [isSubmitting, setIsSubmitting] = useState(false);

    const isEditMode = dossierToEdit !== null;

    useEffect(() => {
        if (dossierToEdit) {
            setFormData(dossierToEdit);
        } else {
            setFormData({ numDossier: '', atelier: '', alias: '', corresp: '', volumePrevue: '', idEtat: 1 });
        }
    }, [dossierToEdit]);
    ...
    return (
        <form onSubmit={handleSubmit} className="flex flex-col h-full">
            <div className="flex justify-between items-center p-4 border-b">
                <h2 className="text-xl font-semibold text-gray-800">{isEditMode ? 'Modifier' : 'Ajouter'} un dossier</h2>
            ...
            <div className="p-6 flex-1 space-y-4 overflow-y-auto">
                <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                    <InputField label="Numéro de dossier" name="numDossier" value={formData.numDossier || ''} onChange={handleChange} required />
                    <InputField label="Alias" name="alias" value={formData.alias || ''} onChange={handleChange} />
                </div>
                <InputField label="Atelier" name="atelier" value={formData.atelier || ''} onChange={handleChange} />
                <InputField label="Correspondant" name="corresp" value={formData.corresp || ''} onChange={handleChange} />
                <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                    <InputField label="Volume Prévu" name="volumePrevue" value={formData.volumePrevue || ''} onChange={handleChange} />
                    <div>
...
const InputField: React.FC<{ label: string, name: string, value: string, onChange: (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement>) => void, type?: string, required?: boolean }> = ({ label, name, ...props }) => (
    <div>
...
const DeletePanel: React.FC<{ dossier: Dossier, onDelete: () => void, onCancel: () => void }> = ({ dossier, onDelete, onCancel }) => {
    ...
    return (
        <div className="flex flex-col h-full">
             <div className="flex items-center p-4 border-b">
                 <h2 className="text-xl font-semibold text-gray-800">Supprimer le dossier</h2>
            </div>
                ...
                <div className="flex items-start">
                    ...
                    <div className="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                        <div className="mt-2">
                            ...
                            Êtes-vous sûr de vouloir supprimer le dossier <strong>{dossier.numDossier}</strong> ? Cette action est irréversible.
                            </p>
                        </div>
                    </div>
                </div>
